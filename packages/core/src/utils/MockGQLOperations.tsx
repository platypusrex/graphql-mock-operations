...
export class MockGQLOperations<TMockGQLOperations extends MockGQLOperationsType<any, any>> {
  private readonly operations: MockGQLOperationsConfig<
    TMockGQLOperations['operations']
  >['operations'];
  private readonly introspectionResult: MockGQLOperationsConfig<
    TMockGQLOperations['operations']
  >['introspectionResult'];
  ...
  private generateProviderProps = ({
    ...
    onResolved,
    ...rest
  }: MockedProviderProps<TMockGQLOperations['state']> &
  ...
  private mapOperations = (
    ...
    return resolvers.reduce((resolver, operation) => {
      ...
      resolver[key as keyof MockGQLOperationsCreate<any, any>] = operation(
        resolverState as unknown as TMockGQLOperations['state']
      )[key];
      return resolver;
  ...
  private createResolvers = (state?: TMockGQLOperations['state']): IResolvers => {
    return [this.operations].reduce((resolvers, operation) => {
      ...
      for (const key of keys as Array<keyof typeof operation>) {
        resolvers[this.generateResolverKey(key)] = this.mapOperations(
          this.operations[key] ?? [],
          state
        );
      }
