...
type RequiredOperationStateObject<TOperationReturn> = {
  state: 'SUCCESS';
...
type NonEmptyStateObject<T extends OperationStateObject<any, any>> = [
...
export const createOperation =
  ...
  (action: Record<keyof TMockOperation, TOperationState[keyof TMockOperation]>) => ({
    [name]: (
      ...
      let currentStateObj =
        typeof state === 'function' ? (state as Function)(parent, variables, context, info) : state;
